...
export function SmartContractsPanel() {
  ...
  const [contracts, setContracts] = React.useState<SmartContractRecord[]>([]);

  const [activeContractId, setActiveContractId] = React.useState<string | null>(
    null,
  );
  ...
  const columns: ColumnDef<SmartContractRow, unknown>[] = [
    ...
    createIdentifierColumn<SmartContractRow>("Loan ID", "loanId"),
    {
      ...
      header: "Loan Value",
      cell: ({ getValue }) => (
        <span>{formatLoanCaseMoney(Number(getValue() ?? 0))}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      header: "LTV",
      cell: ({ getValue }) => (
        <span>{Number(getValue() ?? 0).toFixed(1)}%</span>
      ),
    },
    {
    ...
    },
    {
      ...
      header: "Status ID",
      cell: ({ getValue }) => {
        const badge = getTableStatusBadge(
          getValue() as SmartContractStatus,
        );
        ...
        contract.id === pendingApprovePayload.loanId
          ? {
              ...
              liquidationThresholdAmount:
                pendingApprovePayload.liquidationThresholdAmount,
              disbursedDateLabel:
                pendingApprovePayload.disbursedDateLabel,
              repaymentDueLabel:
                pendingApprovePayload.repaymentDueLabel ?? "",
            }
          : contract,
      ),
    );
