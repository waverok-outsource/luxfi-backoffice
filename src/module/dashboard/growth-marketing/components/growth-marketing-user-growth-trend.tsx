...
import type { GrowthMarketingTrendPoint } from "@/module/dashboard/growth-marketing/data";
...
export function GrowthMarketingUserGrowthTrend({
  data,
  totalUsersLabel,
}: {
  data: GrowthMarketingTrendPoint[];
  totalUsersLabel: string;
}) {
  return (
    <section className="rounded-2xl bg-primary-white p-4 md:p-5">
      <div className="flex flex-wrap items-center justify-between gap-10">
        <h2 className="text-[18px] font-semibold text-text-black">User Growth Trend</h2>

        <div className="rounded-2xl flex-1 bg-primary-grey-undertone px-4 py-3 text-sm font-semibold text-text-black">
          {totalUsersLabel}
        </div>

        <div className="flex items-center gap-5">
          <LegendItem color="#2A8CFF" label="Verified" />
          <LegendItem color="#C8A159" label="Unverified" />
        </div>
      </div>
      ...
      <div className="mt-6 h-[420px] min-w-0">
        ...
        >
          <ComposedChart data={data} margin={{ top: 10, right: 6, left: -10, bottom: 8 }}>
            <defs>
              <linearGradient id="verifiedAreaGradient" x1="0" y1="0" x2="0" y2="1">
            ...
            <Area
              type="monotone"
              dataKey="verified"
              fill="url(#verifiedAreaGradient)"
              stroke="#2A8CFF"
              strokeWidth={2.5}
              dot={false}
              activeDot={{ r: 4, fill: "#2A8CFF" }}
            />
            <Bar dataKey="unverified" fill="#C8A159" radius={[6, 6, 0, 0]} barSize={28} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </section>
