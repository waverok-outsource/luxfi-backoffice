...
| **Asset Management** | ✅ Complete | ✅ Full CRUD | API (classes, categories, assets, providers) | Mostly integrated | [0008](adr/0008-asset-management.md) |
| **Asset Loans** | ✅ Complete | ⚠️ Contracts confirmed, not yet built | Requests/approve-reject/activity-log endpoints confirmed via Postman; Repayments/Disbursements tabs and 5 metric cards have no dedicated endpoint (only embedded per-loan data exists) | Mocked — endpoints confirmed, build pending | [0009](adr/0009-asset-loans.md) |
...
| **Help & Support** | ✅ Complete | ✅ Built | API (tickets, password reset requests) — wired per [0019](adr/0019-support-tickets-api-integration.md); ticket review (mark resolved/pending) is fully broken against production, a backend validator rejects the documented request body, see [bug report 0001](bug-reports/0001-support-tickets-backend-issues.md#bug-2-ticket-review-patch-rejects-a-validly-populated-status-field); stats endpoint has no sampled response, metrics stay hardcoded | Partially integrated — blocked by backend bug, not frontend | [0014](adr/0014-help-support.md), [0019](adr/0019-support-tickets-api-integration.md) |
...
- [ ] `GET /v1/support/stats` has no sampled response — Help & Support's 3 metric cards stay hardcoded until a shape is available
- [ ] Out of scope this round (no UI consumer exists): create-ticket flow, ticket categories endpoint

### Customer Module — remaining tabs (KYC, Asset Portfolio, Asset Loans, Transactions, Smart Contracts)

ADR 0006 only documented 3 of the customer detail page's 7 tabs (Portfolio, Contracts, Session Logs — and even those labels don't match the actual 7: KYC & Compliance, Transactions History, Asset Loans, Smart Contracts, Asset Portfolio, Device Session Logs, Support Tickets). Only Device Session Logs is API-wired today; Support Tickets is designed (see above). Status of the rest, per `luxfi.postman_collection.json`:
...
- [ ] **Asset Portfolio** (customer assets) — confirmed endpoints (`GET .../assets`, `GET .../assets/:id`, `PATCH .../assets/:id/review`), fields map cleanly onto the existing mock UI. Open question: the mocked UI has a `"notVerified"` status distinct from `"pending"`; only `pending`/`verified`/`rejected` were observed in samples — confirm whether a fourth state exists before wiring the badge config.
- [ ] **Asset Loans** (embedded) — confirmed via the same `/v1/loans` resource as the dashboard-level Asset Loans module (see below); scope this alongside that module rather than separately, since it's the same backend resource filtered to one customer (`GET /v1/customers/:customerId/loans`).
...
Four tabs, fully typed row definitions, all hardcoded. **2026-08-11:** Backend contracts confirmed via `luxfi.postman_collection.json` for 3 of 4 pieces — not yet built:
...
### 1. In progress: Support Tickets & Password Reset Requests
Cleanest, most-fully-sampled contract of the newly-confirmed endpoints, no data-model mismatches to design around. Design complete ([ADR 0019](adr/0019-support-tickets-api-integration.md)), handed off to DeepSeek (`docs/implementation.md`).

### 2. Next: Customer Module remaining tabs
Asset Portfolio and the embedded Asset Loans tab have clean, confirmed contracts. KYC & Compliance is confirmed but needs a real UI redesign first (fixed 3-tier pipeline → dynamic N-tier model matching `GET /v1/kyc-tiers`) — scope that as its own design pass, not a rewiring task. Transactions History and Smart Contracts stay mocked; no endpoint exists for either.

### 3. Then: Asset Loans dashboard module
Loan Requests, approve/reject, and Activity Logs are confirmed and buildable now. Repayments and Disbursements tabs are blocked on a backend decision (no admin-wide list endpoint exists, only per-loan embedded data) — flag to backend before or during this phase rather than building around it silently.

### 4. Resolve Asset Management API gaps
