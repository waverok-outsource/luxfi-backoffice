...
**2026-09-09:** Marketplace's LuxFi Listing "Unlist Asset" action — flagged in [ADR 0018](adr/0018-marketplace-api-integration.md) as kept-but-disabled with no backend endpoint — is now wired to a confirmed contract: `PATCH /v1/asset-market/:listingId/unlist` with body `{ status: "approved" }`, returning the listing with `listingStatus: "removed"`. `AssetMarketListingStatus` gained the `"removed"` member (status config, modal labels, and the P2P trade-status-history exhaustiveness map all updated); the Asset Listing Details modal's confirm dialog now calls the real endpoint (with a pending spinner) instead of faking success, and the trigger disables once a listing is already removed. `tsc`/`eslint`/`next build` all clean; live smoke test against the real backend still pending.

**2026-09-09:** Fixed the "Class" column/field on Customer Listings, LuxFi Listing, and every asset-details modal (`AssetDetailsPanel`) rendering blank instead of `-`. A live sample confirmed `assetDetails.assetClass` is genuinely omitted by the backend when the underlying asset has no class assigned (not a schema change — `assetDetails.category` can be independently blank too, they're separate taxonomy fields). `AssetMarketAssetDetails.assetClass` is now optional in the type, and all four call sites (`customer-listings-tab.tsx`, `luxfi-listing-tab.tsx`, `asset-panels.tsx`'s Asset Class *and* Asset Category fields) fall back to `-`, matching the ADR 0018 "render gaps as `-`" convention. `tsc`/`eslint`/`next build` all clean.
...
- [x] Unlist Asset — wired 2026-09-09 to `PATCH /v1/asset-market/:listingId/unlist`; see the dated note above.
